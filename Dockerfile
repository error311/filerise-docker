...
# This step updates and upgrades packages, and cleans up caches in one layer.
RUN apt-get update && \
    apt-get upgrade -y && \
    apt-get install -y --no-install-recommends \
      apache2 \
...
# --- Least Privilege ---
# Switch to a non-root user (www-data) so that the container processes run with least privilege.
USER www-data
